...
@Service
@AllArgsConstructor
public class TipoSolicitudFirmaBajaServiceImpl implements TipoSolicitudFirmaService {
  ...
  public String validarSituacion(Optional<RegistradorCivilBean> registradorCivil, ValidaRegCivilRequest request) {
    ...
    if (estado.equals(RegCivilConstant.ESTADO_BAJA_INHABILITADO))
      throw new ApiValidateException(String.format(RegCivilConstant.MSG_REG_CIVIL_INHABILITADO_DESDE,
          registradorCivil.get().getFechaCambioEstado()
              .format(DateTimeFormatter.ofPattern(ConstantUtil.DATE_FORMAT))));
    ...
    return RegCivilConstant.SITUACION_BAJA;
  }
  ...
  public void registrarFicha(RegistrarFichaRequest request) {
    ...
    RegistradorCivilDetalleBean regCivilDetBean = RegistradorCivilDetalleBean.builder()
        ...
        .fechaBaja(Optional.ofNullable(request.getFechaBaja())
    ...
    RegistradorCivilBean regCivilBean = RegistradorCivilBean.builder()
        .numeroDocIdentidad(request.getDni())
        .idTipoDocIdentidad(SolicitudConstant.TIPO_DOC_DNI)
        .fechaCambioEstado(regCivilDetBean.getFechaBaja())
        .estado(request.getCodigoEstadoRegistrador())
    ...
    registradorCivilDao.actualizarBajaPorDni(regCivilBean);
